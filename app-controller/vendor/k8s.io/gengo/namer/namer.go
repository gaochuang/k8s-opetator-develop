...
//
// Or if the type, in package App, is "type Bar struct { ... }", then the raw
// namer will return "App.Bar" as the name of the type, and if 'tracker' was
// not nil, will record that package App needs to be imported.
...
type NameStrategy struct {
	...
	// ignored.
	//
	// For example, if Ignore words lists "proto" and type App is in
	// pkg/server/frobbing/proto, then a value of 1 will give a type name
	// of FrobbingApp, 2 gives ServerFrobbingApp, etc.
